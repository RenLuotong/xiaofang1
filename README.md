...
- 用于开发测试使用的代码
- 用于本地开发和部署测试环境的项目
- 如果有新功能开发完成或者修改分支上有bug，需要在liaocheng-dev上修改合并代码

### 重大更新历史
-2020/3/25 整理分支，分支重命名为liaocheng-dev



## 项目使用相关技术：
Node.js  React  Ant.Design  webpack
...
node -v查看当前node版本，使用6.5以上版本，低版本部分命令不支持
如何使用node：
cmd，并将路径切换到项目路径

## 二、NPM 相关：

npm是nodejs官方为nodejs定制的一个工具，是Node.js的包管理器，是Node Packaged Modules的简称
新版的nodejs已经集成了npm，无需单独安装
如果已经安装了旧版本的 npm
npm install npm -g 升级npm版本
npm -v 查看当前npm版本
npm -h 查看npm命令的用法
npm教程 https://www.cnblogs.com/penghuwan/p/6973702.html

通过npm可以下载安装nodejs的模块包
下载包常用命令：
npm install
初始化项目（git库中不上传包，初次import项目时或package.json被修改时执行此命令），会将package.json中的包安装到node_modules目录中（如果package.json文件被修改，会只下载改动部分）
npm install xxx --save
将xxx的最新版本安装到node_modules目录中，并会在package.json的dependencies属性下添加xxx
npm install xxx --save-dev
将xxx的最新版本安装到node_modules目录中，并会在package.json的devDependencies属性下添加xxx
npm install --production
安装package.json中dependencies里面的包，不会安装devDependencies里面的包

使用淘宝NPM镜像，用cnpm命令行工具代替默认的npm加快包下载速度
npm install -g cnpm --registry=https://registry.npm.taobao.org
以后可以使用 cnpm install进行下载（只有下载包的时候用cnpm替换npm，其他情况下仍使用npm）

## 三、React 相关：

react官方文档 https://doc.react-china.org/docs/hello-world.html（重点）
...
JSX：JSX是JavaScript语法的扩展，React开发不一定使用JSX，但建议使用JSX
组件：通过React构建组件，使代码更容易复用，能够很好的应用在大项目的开发中
单向响应的数据流：React实现了单向响应的数据流，从而减少了重复代码，比传统数据绑定更简单

JSX特点：
JSX执行更快，它在编译为JavaScript代码后进行了优化
它是类型安全的，在编译过程中就能发现错误
使用JSX编写模板更加简单快速
JSX教程 https://blog.csdn.net/a153375250/article/details/53434299

## 四、Ant.Design 相关：

ant.design官网 https://ant.design/docs/react/introduce-cn
Ant Design 是一种UI设计语言，是一套适用于React的组件库，适用于开发企业级后台产品
...
webpack处理应用程序时，它会递归地构建一个依赖关系图，其中包含应用程序需要的每个模块，然后将所有这些模块打包成一个或多个bundle
在webpack看来一切都是模块，包括你的JavaScript代码，也包括CSS以及图片等，通过合适的loaders，它们都可以被当做模块被处理

## 六、如何运行项目：

前端的项目往往依赖了很多打包、部署工具，在这些多打包、部署工具里，都有这各自的命令
所有将这些命令都封装到npm scripts里，即package.json文件的scripts里：
"scripts": {
    "build": "webpack --profile --mode development",
    "start": "webpack-dev-server --mode development --profile --open",
    "pro": "webpack --mode production"
}
npm会默认设置一些script的值，比如start和install，不过也可以覆盖这2个值
