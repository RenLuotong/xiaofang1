...
class App extends Component {
  ...
  fetch = (
    ...
    $.ajax({
      ...
      url:
        SERVER +
        "chaxunbaoguanguishiyongjilu/all?page=" +
        page +
        "&size=" +
        size +
        "&baoguanguimingcheng=" +
      ...
      success: function (data) {
        ...
        for (let i = 0; i < data.data.content.length; i++) {
          ...
          data.data.content[i]["shiyongshijian"] = moment(
            data.data.content[i]["shiyongshijian"]
  ...
  render() {
    const yingquOptions = this.state.yingquList.map((item) => (
      <Select.Option key={item["id"]} value={item["yingqubianhao"]}>
        {item["mingcheng"]}
      </Select.Option>
    ));
      ...
      <div>
        ...
        >
          ...
          <FormItem label="营区名称">
            <Select
              style={{ width: 200 }}
              value={yingqumingcheng}
              onChange={this.getyingquList.bind(this)}
            >
