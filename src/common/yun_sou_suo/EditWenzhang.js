...
class EditWenzhang extends Component{
  constructor (props) {
    super(props)
    this.state={
      loading: false,
  ...
  getWenzhangInfo = () => {
    let _this = this;
    let data = {
      wenzhangbianhao: this.props.match.params.bianhao
    }
    $.ajax({
      type:'GET',
      ...
      success: function (data) {
        ...
        _this.setState({
          dataInfo: data.data,
          imageUrl: data.data.fengmian,
          biaoti: data.data.biaoti,
          zilanmubianhao: data.data.zilanmubianhao,
        });
      }
    });
  }
  ...
  tianshuFromFn = (e) => {
    let arr = [];
    this.state.dataInfo.neirong.map(item => {
      arr.push(item);
    })
    arr.splice(Number(e.target.name),1,{ ...this.state.dataInfo.neirong[Number(e.target.name)],shujuxiang: e.target.value});
    this.setState({
      dataInfo:{
        ...this.state.dataInfo,
        neirong: arr
  ...
  titleFn = (e) => {
    ...
    arr.splice(Number(e.target.name),1,{ ...this.state.dataInfo.neirong[Number(e.target.name)],neirong: e.target.value});
    this.setState({
      dataInfo:{
        ...this.state.dataInfo,
        neirong: arr
  ...
  addTitles = () => {
    let arr = [];
    if(this.state.dataInfo.neirong === undefined){
  ...
  addWenzhan = () => {
    ...
    if(this.state.fileList.length === 0){
      data['fengmian'] = this.state.imageUrl
    }
    if(this.state.dataInfo.neirong.length > 0){
      data['neirong'] = this.state.dataInfo.neirong
    }else {
      data['neirong'] = []
    }
    ...
    $.ajax({
      type : 'post',
      url : SERVER + "yunsousuo/xiugai-wenzhang",
  ...
  render () {
    ...
    const titlesList = this.state.dataInfo.neirong && this.state.dataInfo.neirong.length > 0 && this.state.dataInfo.neirong.map((item, index) => {
      return(
    ...
    return (
      <div style={{overflowY: 'scroll',height: 'calc(100vh - 90px)',padding: '5%'}}>
        ...
        <Select style={{ width: 200 }} allowClear value={this.state.zilanmubianhao} onChange={this.onChangeSelect}>
          {
