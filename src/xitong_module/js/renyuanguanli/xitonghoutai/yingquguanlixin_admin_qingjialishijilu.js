...
import yingquguanlixin_admin_qingjialishijiluxiangqing from "./yingquguanlixin_admin_qingjialishijiluxiangqing";
        ...
        <Switch>
          ...
          <Route
            path={
              ...
              "/yingquguanlixin_admin_qingjialishijiluxiangqing/:processInstanceId"
            }
            component={yingquguanlixin_admin_qingjialishijiluxiangqing}
          />
        </Switch>
      </div>
    );
...
class App extends Component {
  ...
  fetch = (
    ...
    if (
      typeof qingjialeixing !== "undefined" &&
      qingjialeixing !== "-1" &&
      qingjialeixing !== "" &&
      qingjialeixing !== null
    ) {
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        for (let i = 0; i < data.data.content.length; i++) {
          ...
          if (
            Date.parse(
              data.data.content[i].form.jihuaxiaojiashijian.valueOf()
            ) < current.valueOf() &&
            data.data.content[i].form.shenqingdanzhuangtai == "假期中"
          ) {
            data.data.content[i].form.shenqingdanzhuangtai = "未销假";
          }
          data.data.content[i].form.jihuaqingjiashijian = moment(
            data.data.content[i].form.jihuaqingjiashijian
          ).format("YYYY-MM-DD HH:mm:ss");
          data.data.content[i].form.jihuaxiaojiashijian = moment(
            data.data.content[i].form.jihuaxiaojiashijian
          ).format("YYYY-MM-DD HH:mm:ss");
          for (
            let j = 0;
            j < data.data.content[i].historicUserTaskInstanceList.length;
            j++
          ) {}
  ...
  output() {
    ...
    $.ajax({
      ...
      url:
        SERVER +
        "qingjiajiluchaxunByJuese?paze=0&size=10000&qingjiarenxingming=" +
        qingjiarenxingming +
        "&kaishishijian=" +
        kaishishijian +
        "&jieshushijian=" +
      ...
      success: function (data) {
        ...
        for (let i = 0; i < data.data.content.length; i++) {
          ...
          data.data.content[i]["qingjiakaishishijian"] = moment(
            data.data.content[i]["qingjiakaishishijian"]
          ).format("YYYY-MM-DD HH:mm:ss");
          data.data.content[i]["qingjiajieshushijian"] = moment(
            data.data.content[i]["qingjiajieshushijian"]
          ).format("YYYY-MM-DD HH:mm:ss");
          let obj = {};
  ...
  qingjiatongguo(
    ...
    if (
      shijiqingjialeixing == "公差" ||
      shijiqingjialeixing == "培训" ||
      shijiqingjialeixing == "轮休" ||
      shijiqingjialeixing == "事假（短）"
    ) {
      panduanqingjia = "短假";
    }
    const THE = this;
  ...
  tijiao() {
    ...
    if (
      chaosongren !== "" &&
      chaosongren !== null &&
      chaosongren !== "undefined"
    ) {
      variables.chaosongrenyuanxingming =
        yuanchaosongrenyuanxingming + "," + chaosongren.split("+")[0];
      variables.chaosongrenyuanbianhao =
        yuanchaosongrenyuanbianhao + "," + chaosongren.split("+")[1];
    }
    $.ajax({
      type: "POST",
      url: SERVER + "activiti/completeTask?taskId=" + taskId,
    ...
    const columns = [
      ...
      //     }
      {
        ...
        render: (text, record, index) => {
          if (record.form.shenqingdanzhuangtai == "请假申请中") {
            return (
              <span>
                <Popconfirm
                  placement="topLeft"
                  title="确认要通过这条请假申请?"
                  onConfirm={this.qingjiatongguo.bind(
                    this,
                    record["historicUserTaskInstanceList"][
                      record.historicUserTaskInstanceList.length - 1
                    ].taskInstanceId,
                    record["form"].shijiqingjialeixing,
                    record["historicUserTaskInstanceList"][
                      record.historicUserTaskInstanceList.length - 1
                    ].userTaskExtendedAttributes,
                ...
                <Popconfirm
                  placement="topLeft"
                  title="确认要拒绝这条请假申请?"
                  onConfirm={this.qingjiajujue.bind(
                    this,
                    record["historicUserTaskInstanceList"][
                      record.historicUserTaskInstanceList.length - 1
                    ].taskInstanceId
                ...
                <Link
                  to={
                    ...
                    "/yingquguanlixin_admin_qingjialishijiluxiangqing/" +
          ...
          } else if (record.form.shenqingdanzhuangtai == "延期申请中") {
            return (
              <span>
                <Popconfirm
                  placement="topLeft"
                  title="确认要通过这条延期申请?"
                  onConfirm={this.yanqitongguo.bind(
                    this,
                    record["historicUserTaskInstanceList"][
                      record.historicUserTaskInstanceList.length - 1
                    ].taskInstanceId,
                    record["form"].yanqishijian
                  )}
                  okText="确认"
                  cancelText="取消"
                >
          ...
          } else if (record.form.shenqingdanzhuangtai == "销假申请中") {
            return (
              <span>
                <Popconfirm
                  placement="topLeft"
                  title="确认要通过这条销假申请?"
                  onConfirm={this.xiaojiatongguo.bind(
                    this,
                    record["historicUserTaskInstanceList"][
                      record.historicUserTaskInstanceList.length - 1
                    ].taskInstanceId
            ...
            record["form"].shenqingdanzhuangtai == "请假同意" ||
            record["form"].shenqingdanzhuangtai == "假期中" ||
            record["form"].shenqingdanzhuangtai == "未销假"
          ) {
            return (
              <span>
                <a
                  href={
                    SERVER +
                    "generateReport?processInstanceId=" +
                    record["processInstanceId"]
                  }
                  title="打印"
                  target="_blank"
