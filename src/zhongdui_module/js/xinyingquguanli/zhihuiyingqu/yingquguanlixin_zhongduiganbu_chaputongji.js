...
class App extends React.Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            xungengList: [],
            //按计划共有10次查铺，8次准时，2次未到;准时率80%，未到率20%
            xungengInfo: {
                zongcishu: 0,
                zhunshi: 0,
                chidao: 0,
            ...
            success: function (data) {
                ...
                for (let i = 0; i < data.data.content.length; i++) {
                	...
                	if (data.data.content[i]['xgmc'] == null) {
                        ...
                        obj['查铺计划名称'] = data.data.content[i]['xgmc'];
                    }
                    if (data.data.content[i]['xgdd'] == null) {
                        obj['查铺计划地点'] = '';
                    } else {
                        obj['查铺计划地点'] = data.data.content[i]['xgdd'];
                    }
                    if (data.data.content[i]['danqianriqi'] == null) {
                        obj['查铺计划时间'] = '';
                    } else {
                        obj['查铺计划时间'] = data.data.content[i]['danqianriqi'];
                    }
                    if (data.data.content[i]['num'] == null) {
                        obj['查铺执行次数'] = '';
                    } else {
                        obj['查铺执行次数'] = data.data.content[i]['num'];
                    }
                    obj['查铺人'] = '';
                	obj['查铺地点'] = '';
                	obj['查铺时间'] = '';
                    ...
                    if (data.data.content[i]['children'] != null) {
                    	for (let a = 0; a < data.data.content[i].xgjlDTOList.length; a++) {
		                	...
		                	if (data.data.content[i].xgjlDTOList[a]['xingming'] == null) {
		                        ...
		                        obja['查铺人'] = data.data.content[i].xgjlDTOList[a]['xingming'];
		                    }
		                    if (data.data.content[i].xgjlDTOList[a]['xungendidian'] == null) {
		                        obja['查铺地点'] = '';
		                    } else {
		                        obja['查铺地点'] = data.data.content[i].xgjlDTOList[a]['xungendidian'];
		                    }
		                    if (data.data.content[i].xgjlDTOList[a]['shijian'] == null) {
		                        obja['查铺时间'] = '';
		                    } else {
		                        obja['查铺时间'] = data.data.content[i].xgjlDTOList[a]['shijian'];
		                    }
		                    list.push(obja);
                    	}
                    }
                ...
                if (_data == null || _data.length == 0) {
                    message.warning("当前查铺统计没有数据！");
                    return;
                }
				let _headers = ['查铺计划名称','查铺计划地点','查铺计划时间','查铺执行次数','查铺人','查铺地点','查铺时间'];
    ...
    render() {
        ...
        const columns = [
        	{title: '查铺计划名称', dataIndex: 'xgmc'},
        	{title: '查铺计划地点', dataIndex: 'xgdd'},
        	{title: '查铺计划时间', dataIndex: 'danqianriqi'},
        	{title: '查铺执行次数', dataIndex: 'num'},
        	{title: '查铺人', dataIndex: 'xingming'},
        	{title: '查铺地点', dataIndex: 'xungendidian'},
        	{title: '查铺时间', dataIndex: 'shijian'},
        ];
        ...
        return (
            <div>
                ...
                <p className="xungengp">
                    按计划共有 {xungengInfo['zongcishu']} 次查铺，{xungengInfo['zhunshi']} 次已到，{xungengInfo['chidao']} 次未到
                </p>
                <p className="xungengp">
