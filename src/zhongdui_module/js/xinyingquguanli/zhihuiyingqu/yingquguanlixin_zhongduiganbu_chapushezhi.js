...
class App extends React.Component {
    ...
    output()  {
        let leixing = "查铺";
        let _headers = ['查铺名称', '查铺日期范围', '查铺星期范围', '查铺时间范围', '巡更地点']
        $.ajax({
            type:'get',
            ...
            success: function (data) {
                ...
                for (let i = 0; i < data.data.content.length; i++) {
                    ...
                    if (data.data.content[i]['xungengmingcheng'] == null) {
                        ...
                        obj['查铺名称'] = data.data.content[i]['xungengmingcheng'];
                    }
                    if (data.data.content[i]['kaishiriqi'] == null && data.data.content[i]['jieshuriqi'] == null) {
                        obj['查铺日期范围'] = '';
                    } else {
                        obj['查铺日期范围'] = data.data.content[i]["kaishiriqi"] + " ~ " + data.data.content[i]["jieshuriqi"];
                    }
                    ...
                    if (data.data.content[i]['shijianfanweikaishi'] == null && data.data.content[i]['shijianfanweijieshu'] == null) {
                        ...
                        obj['查铺时间范围'] = data.data.content[i]["shijianfanweikaishi"] + " ~ " + data.data.content[i]["shijianfanweijieshu"];
                    }
    ...
    render() {
        ...
        const columns = [{
            ...
            render: (text, record) => (
                <span>
                    <Popconfirm placement="topLeft" title="确认要删除该查铺计划?"
        ...
        return (
            <div>
                <Button style={{margin:5}}>
                    <Link to="/yingquguanlixin_zhongduiganbu_chaputianjia"><Icon type="plus" /><span>查铺计划</span></Link>
                </Button>
                ...
                <Table
                    columns={columns}
                    dataSource={this.state.chapuList}
