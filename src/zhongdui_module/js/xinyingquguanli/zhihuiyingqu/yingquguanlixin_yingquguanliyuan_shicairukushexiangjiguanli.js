...
import yingquguanlixin_yingquguanliyuan_shicairukushexiangjixiugai from './yingquguanlixin_yingquguanliyuan_shicairukushexiangjixiugai';
import yingquguanlixin_yingquguanliyuan_shicairukushexiangjiitianjia from './yingquguanlixin_yingquguanliyuan_shicairukushexiangjiitianjia';
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (_data == null || _data.length == 0) {
                    message.warning("食材入库摄像机管理列表没有数据！");
                    return;
                }
                ...
                // 导出 Excel
                XLSX.writeFile(wb, '食材入库摄像机管理.xlsx');
            }
        });
    ...
    render() {
        const columns = [{
          ...
          width: '10%'
            ...
            render: (text, record) => (
                <span>
                    <Popconfirm placement="topLeft" title="确认要删除该食材入库摄像机?"
                                onConfirm={this.toDelete.bind(this, record['id'])} okText="确认" cancelText="取消">
                        <a>删除</a>
                    </Popconfirm>
                    <Divider type="vertical"/>
                    <Link to={'yingquguanlixin_zhongduiganbu_kaoqinjiguanli/yingquguanlixin_yingquguanliyuan_shicairukushexiangjixiugai/' + record['id']}>修改</Link>
                </span>
            ),
        },];
        ...
        return (
            <div>
                <Button style={{margin: 5}}>
                    <Link to={'yingquguanlixin_zhongduiganbu_kaoqinjiguanli/yingquguanlixin_yingquguanliyuan_shicairukushexiangjiitianjia'}><Icon type="plus"/><span>添加食材入库摄像机</span></Link>
                </Button>
