...
import {
    ...
    Button,
    Table,
    message, Select, Divider, Popconfirm, Modal, Card, Tree
} from 'antd';
    ...
    render() {
        ...
        let shenpirenyuanOptions = this.state.shenpirenyuanList.map(item =>
            <Select.Option key={item['key']}
                           value={item['xingming'] + '+' + item['renyuanbianhao']}>{item['xingming']}</Select.Option>
        );
            ...
            // },
            {
                ...
                render: (text, record, index) => {
                    if (record['anniushenpizhuangtai'] == "外出未审批" && record['form'].jihuachucheshijian !== undefined && record['form'].jihuaguiduishijian !== undefined) {
                        return (
                            <span>
                            <Popconfirm placement="topLeft" title="确认要通过这条用车申请?"
                                        onConfirm={this.yongchetongguo.bind(this, record['historicUserTaskInstanceList'][record.historicUserTaskInstanceList.length - 1].taskInstanceId, record['historicUserTaskInstanceList'][record.historicUserTaskInstanceList.length - 1].userTaskExtendedAttributes, record['form'].yichuxiaqu, record['form'].faqirenliuchengjueses,record['form'].chaosongrenyuanxingming,record['form'].chaosongrenyuanbianhao)}
                                        okText="确认" cancelText="取消">
					        	<a>通过</a>
					    	</Popconfirm>
					    	<Divider type="vertical"/>
					    	<Popconfirm placement="topLeft" title="确认要拒绝这条用车申请?"
                                        onConfirm={this.yongchejujue.bind(this, record['historicUserTaskInstanceList'][record.historicUserTaskInstanceList.length - 1].taskInstanceId)}
                                        okText="确认" cancelText="取消">
					        	<a>拒绝</a>
					    	</Popconfirm>
                            <Divider type="vertical"/>
                            <Link
                                to={'yingquguanlixin_zuzhiyuangong_gongyongchelishijilu/yingquguanlixin_cheliangguanliyuan_yongchelishijiluqingkuang/' + record['processInstanceId']}>详情</Link>
                    <Divider type="vertical"/>
                    <Link
                        to={'yingquguanlixin_zuzhiyuangong_gongyongchelishijilu/yingquguanlixin_cheliangguanliyuan_yongchelishijiluxiangqing/' + record['form'].chepaihaoma + '/' + record['form'].jihuachucheshijian + '/' + record['form'].jihuaguiduishijian}>轨迹</Link>
                       </span>
                        )
                    } else if (record['anniushenpizhuangtai'] == "归队未审批") {
                        return (
                            <span>
                            <Popconfirm placement="topLeft" title="确认要通过这条归队申请?"
                                        onConfirm={this.guiduitongguo.bind(this, record['historicUserTaskInstanceList'][record.historicUserTaskInstanceList.length - 1].taskInstanceId)}
                                        okText="确认" cancelText="取消">
					        	<a>通过</a>
					    	</Popconfirm>
					    	<Divider type="vertical"/>
