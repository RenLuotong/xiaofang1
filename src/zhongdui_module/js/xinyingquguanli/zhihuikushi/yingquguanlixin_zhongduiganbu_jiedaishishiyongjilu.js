...
class App extends Component {
    ...
    fetch = (params = {rows: this.state.pagination.pageSize,
        ...
        $.ajax({
            type:'get',
            url: SERVER + "huoquJiashujiedaishiFangjianshiyongjilu?page="+page+"&size="+size+"&kaishishijian="+kaishishijian+"&jieshushijian="+jieshushijian+"&shenpizhuangtai="+shenpizhuangtai+"&fangjianmingcheng="+fangjianmingcheng+"&fangjianZhuangtai="+fangjianzhuangtai+paixu,
            success: function (data) {
                let list = [];
                if (data.status != 0) {
                    message.warning(data.message);
    ...
    tongyi(id){
        const THE = this;
        let shenpizhuangtai = 'SHIYONGZHONG';
        ...
        $.ajax({
            type:'POST',
            url: SERVER + "shenheJiashujiedaishiYongfangshenqing",
        ...
        const columns = [{
            ...
            render: (text, record, index) => {
                if (record['shenpizhuangtai'] == "申请中") {
                    return (
                        <span>
					      	<Popconfirm placement="topLeft" title="确认要同意该接待室使用申请?" onConfirm={this.tongyi.bind(this, record['id'])} okText="确认" cancelText="取消">
					        	<a>通过</a>
					    	</Popconfirm>
					    	<Divider type="vertical"/>
					    	<Popconfirm placement="topLeft" title="确认要拒绝该接待室使用申请?" onConfirm={this.jujue.bind(this, record['id'])}  okText="确认" cancelText="取消">
					        	<a>拒绝</a>
					    	</Popconfirm>
                       </span>
                    )
