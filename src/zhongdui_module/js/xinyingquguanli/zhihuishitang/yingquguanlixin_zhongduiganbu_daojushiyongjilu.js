...
class App extends Component {
    ...
    fetch = (params = {rows: this.state.pagination.pageSize,
      ...
      let size = params.rows === undefined ? 10 : params.rows;
        ...
        $.ajax({
            type:'get',
            url: SERVER + "chaxundaojushiyongjiluNew?page="+page+"&size="+size+"&kaishishijian="+kaishishijian+"&jieshushijian="+jieshushijian+"&chakanfanwei="+chakanfanwei+"&jigoudaima="+jigoudaima,
            success: function (data) {
                let list = [];
                if (data.status != 0) {
                    message.warning(data.message);
