...
class App extends Component {
    ...
    fetch = (params = {rows: this.state.pagination.pageSize,
      ...
      let size = params.rows === undefined ? 10 : params.rows;
        $.ajax({
            type:'get',
            url: SERVER + "chaxungaojingjiluNew?page="+page+"&size="+size+"&shicaimingcheng="+shicaimingcheng+"&kaishishijian="+kaishishijian+"&jieshushijian="+jieshushijian+"&chakanfanwei="+chakanfanwei+"&jigoudaima="+jigoudaima,
            success: function (data) {
                let list = [];
                if (data.status != 0) {
                    message.warning(data.message);
    ...
    render() {
        ...
        const columns = [{
          ...
          width: '20%'
            ...
            title:'告警库存数量',
            dataIndex:'gaojingkucunshuliang',
        },{
            title:'告警阈值',
