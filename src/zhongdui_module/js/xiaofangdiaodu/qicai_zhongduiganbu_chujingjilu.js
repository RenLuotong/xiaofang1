...
import {message, Button, Table, Form, Select, DatePicker, Icon, Tabs, Card, Input, Divider, Popconfirm} from 'antd';
import {Link, Route ,Switch} from 'react-router-dom';
import moment from 'moment';
import ReactEchartsCore from 'echarts-for-react/';
import echarts from 'echarts/lib/echarts';
import 'echarts/lib/chart/bar';
import chujingjiluxiangqing from './qicai_zhongduiganbu_chujingjiluxiangqing';
import qicai_zhongduiganbu_chujingrenyuanxiangqing from './qicai_zhongduiganbu_chujingrenyuanxiangqing';
import qicai_zhongduiganbu_chujingcheliangxiangqing from './qicai_zhongduiganbu_chujingcheliangxiangqing';
        ...
        return(
            <div>
                <Switch>
                    ...
                    <Route path = {this.props.match.path +'/chujingjiluxiangqing/:id'} component = {chujingjiluxiangqing} />
                    <Route path = {this.props.match.path +'/qicai_zhongduiganbu_chujingrenyuanxiangqing/:renyuanbianhao'} component = {qicai_zhongduiganbu_chujingrenyuanxiangqing} />
                    <Route path = {this.props.match.path +'/qicai_zhongduiganbu_chujingcheliangxiangqing/:cheliangbianhao'} component = {qicai_zhongduiganbu_chujingcheliangxiangqing} />
                </Switch>
            </div>
        );
    }
...
class App extends Component {
    ...
    fetch = (params = {rows: this.state.pagination.pageSize,
      ...
      let size = params.rows === undefined ? 10 : params.rows;
        ...
        $.ajax({
            type:'GET',
            url: SERVER + "getChujingjilu?kaishishijian="+kaishishijian+"&jieshushijian="+jieshushijian+"&page="+page+"&size="+size+"&sort=id,desc",
            success: function (data) {
                ...
                for (let i = 0; i < data.data.content.length; i++) {
                    ...
                    if (data.data.content[i]["chujingshijian"] != null) {
                        data.data.content[i]["chujingshijian"] = moment(data.data.content[i]['chujingshijian']).format('YYYY-MM-DD HH:mm:ss');
                    }
    ...
    fetchrenyuan = (params = {rows: this.state.paginationrenyuan.pageSize,
      ...
      let size = params.rows === undefined ? 10 : params.rows;
        ...
        $.ajax({
            type:'GET',
            url: SERVER + "chujingjilu/chujingrenyuan?page="+page+"&size="+size+"&kaishishijian="+kaishishijian+"&jieshushijian="+jieshushijian+"&sort=id,desc",
            success: function (data) {
                let list = [];
                if (data.status != 0) {
                    message.warning(data.message);
    ...
    fetchcheliang = (params = {rows: this.state.paginationcheliang.pageSize,
      ...
      let size = params.rows === undefined ? 10 : params.rows;
        ...
        $.ajax({
            type:'GET',
            url: SERVER + "chujingjilu/chujingcheliang?page="+page+"&size="+size+"&kaishishijian="+kaishishijian+"&jieshushijian="+jieshushijian+"&sort=id,desc",
            success: function (data) {
                let list = [];
                if (data.status != 0) {
                    message.warning(data.message);
    ...
    fetchtongji = () => {
        ...
        $.ajax({
            type:'GET',
            url: SERVER + "chujingjilu/chujinggtongji?kaishishijian="+kaishishijian+"&jieshushijian="+jieshushijian+"&chujingleixing="+chujingleixing,
            success: function (data) {
                if (data.status != 0) {
                    message.warning(data.message);
                    return;
    ...
    componentWillMount(){
        ...
        const {pagination,activeKey,paginationcheliang,paginationrenyuan} = View;
        this.setState({
            activeKey: activeKey,
        });
    ...
    render() {
        ...
        let a ='出警总次数' + " \n"+ " \n" + '出警次数' + this.state.chujingzongcishu + '次'+ " \n"+ " \n" + '出警车次' + this.state.chujingzongcheliang + '次'+ " \n"+ " \n" + '出警人次' + this.state.chujingzongrenshu + '次';
        ...
        const columns = [{
          ...
          width: '10%'
            ...
            width: '10%'
                ...
                title: '报警人',
                dataIndex: 'baojingren',
            width: '10%'
            }, {
                title: '报警人联系方式',
                dataIndex: 'baojingrenlianxifangshi',
            width: '10%'
            }, {
                title: '灾害地址',
            ...
            },
            {
              ...
              width: '10%'
                ...
                render: (text, record, index) => {
                    return (
                        <span>
                		<Link to={this.props.match.url + '/chujingjiluxiangqing/' + record['id']}>详情</Link>
			    	</span>
                    )
                },
            }];
        ...
        const renyuancolumns = [
          {
            ...
            width: '10%'
              ...
              render: (text, record, index) => (
                  <span>
                <Link to={this.props.match.url+'/qicai_zhongduiganbu_chujingrenyuanxiangqing/'+record['renyuanbianhao'] }>详情</Link>
            </span>
              ),
          }
        ];
        ...
        const cheliangcolumns = [{
          ...
          width: '10%'
            ...
            render: (text, record, index) => (
                <span>
			      	<Link to={this.props.match.url+'/qicai_zhongduiganbu_chujingcheliangxiangqing/'+record['cheliangbianhao'] }>详情</Link>
			    </span>
            ),
        }];
        ...
        const { startValue, endValue,defaultStarDate,defaultEndDate,startValuetongji, endValuetongji,startValuerenyuan, endValuerenyuan,startValuecheliang, endValuecheliang,heighttemp} = this.state;
                ...
                <Tabs activeKey={this.state.activeKey} onChange={this.callback.bind(this)}>
                            ...
                            <FormItem label="出警类型">
                                {getFieldDecorator('chujingleixing')(
                                    <Select style={{width:200}}>
                                        ...
                                        <Select.Option key="火灾" value="火灾">火灾</Select.Option>
                                        <Select.Option key="抢险" value="抢险">抢险</Select.Option>
                                        <Select.Option key="社会救助" value="社会救助">社会救助</Select.Option>
                                        <Select.Option key="增援" value="增援">增援</Select.Option>
                                    </Select>
                                )}
                            </FormItem>
                            <FormItem>
                    ...
                    <Tabs.TabPane tab="出警人员" key="3">
                        <Form onSubmit={this.handleSubmit} layout="inline" style={{margin:5}}>
                            ...
                            {/*</FormItem>*/}
                            <FormItem>
                                <Button type="primary" onClick={this.searchrenyuan.bind(this)}>
                                    <Icon type="search" />搜索
                                </Button>
                            </FormItem>
                        </Form>
                    ...
                    <Tabs.TabPane tab="出警车辆" key="4">
                        <Form onSubmit={this.handleSubmit} layout="inline" style={{margin:5}}>
                            ...
                            {/*</FormItem>*/}
                            <FormItem>
                                <Button type="primary" onClick={this.searchcheliang.bind(this)}>
                                    <Icon type="search" />搜索
                                </Button>
                            </FormItem>
                        </Form>
