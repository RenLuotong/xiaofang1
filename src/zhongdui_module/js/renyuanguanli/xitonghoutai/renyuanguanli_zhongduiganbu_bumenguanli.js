...
import renyuanguanli_zhongduiganbu_bumenguanlixiangqing from './renyuanguanli_zhongduiganbu_bumenguanlixiangqing';
import renyuanguanli_zhongduiganbu_bumenxiugai from './renyuanguanli_zhongduiganbu_bumenxiugai';
import renyuanguanli_zhongduiganbu_bumentianjia from './renyuanguanli_zhongduiganbu_bumentianjia'
        ...
        return (
            <div>
                <Switch>
                    ...
                    <Route path={this.props.match.path + '/renyuanguanli_zhongduiganbu_bumenguanlixiangqing/:id'}
                           component={renyuanguanli_zhongduiganbu_bumenguanlixiangqing}/>
                    <Route path={this.props.match.path + '/renyuanguanli_zhongduiganbu_bumentianjia'}
                           component={renyuanguanli_zhongduiganbu_bumentianjia}/>
                    <Route path={this.props.match.path + '/renyuanguanli_zhongduiganbu_bumenxiugai/:id'}
                           component={renyuanguanli_zhongduiganbu_bumenxiugai}/>
                </Switch>
            </div>
        );
    }
...
class App extends React.Component {
    ...
    fetch = (params = {
        ...
        $.ajax({
            type: 'get',
            url: SERVER + "tb-bumen?page=" + page + "&size=" + size +"&chakanfanwei="+chakanfanwei+"&jigoudaima="+jigoudaima+ "&sort=paixu,desc",
            success: function (data) {
                let list = [];
                if (data.status != 0) {
                    message.warning(data.message);
        ...
        const columns = [{
          ...
          width: '20%'
            ...
            render: (text, record) => (
                <span>
			      	<Link
                        to={this.props.match.url + '/renyuanguanli_zhongduiganbu_bumenguanlixiangqing/' + record['id']}>详情</Link>
			      	<Divider type="vertical"/>
			      	<Popconfirm placement="topLeft" title="确认要删除该部门?" onConfirm={this.toDelete.bind(this, record['id'])}
                                okText="确认" cancelText="取消">
			        	<a>删除</a>
			    	</Popconfirm>
			      	<Divider type="vertical"/>
			      	<Link
                        to={this.props.match.url + '/renyuanguanli_zhongduiganbu_bumenxiugai/' + record['id']}>修改</Link>
			    </span>
            ),
        }];
                    ...
                    <Form onSubmit={this.handleSubmit} layout="inline" style={{margin: 5}}>
                        ...
                        <FormItem>
                            <Button style={{margin: 5}}>
                                <Link to={this.props.match.url + '/renyuanguanli_zhongduiganbu_bumentianjia'}><Icon
                                    type="plus"/><span>添加部门</span></Link>
                            </Button>
                        </FormItem>
                        {/*<FormItem>*/}
