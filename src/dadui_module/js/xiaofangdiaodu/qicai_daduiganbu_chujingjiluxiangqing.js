...
class App extends Component {
    ...
    getCheliangList = (params = {rows: this.state.cheliangPagination.pageSize,
        ...
        $.ajax({
            type:'GET',
            url: SERVER + "getChujingCheliangjilu/"+chujingjiluId+"?page="+page+"&size="+size,
            success: function (data) {
                let list = [];
                if (data.status != 0) {
                    message.warning(data.message);
    ...
    getRenyuanList = (params = {rows: this.state.renyuanPagination.pageSize,
        ...
        $.ajax({
            type:'GET',
            url: SERVER + "getChujingRenyuanjilu/"+chujingjiluId+"?page="+page+"&size="+size,
            success: function (data) {
                let list = [];
                if (data.status != 0) {
                    message.warning(data.message);
    ...
    getQicaiList = (params = {rows: this.state.qicaiPagination.pageSize,
        ...
        $.ajax({
            type:'GET',
            url: SERVER + "getChujingQicaijilu/"+chujingjiluId+"?page="+page+"&size="+size,
            success: function (data) {
                let list = [];
                if (data.status != 0) {
                    message.warning(data.message);
    ...
    render() {
            ...
            },
            {
                ...
                render: (text, record, index) => {
                    if(record.chujingzhuangtai === "出警中"){
                        return (
                            <span>
                             <Popconfirm placement="topLeft" title="确认出警完成?" onConfirm={this.toPass.bind(this, record['cheliangbianhao'])}
        ...
        let chijingshijian ="";
                   if (chujingjiluInfo.chujingshijian != null && chujingjiluInfo.chujingshijian != "") {
            chijingshijian = moment(chujingjiluInfo.chujingshijian).format('YYYY-MM-DD HH:mm:ss');
        }
        return (
            <div>
                <p>
                ...
                />
                <p>
                    <Icon type="info" style={{color: '#1890ff'}}/>人员列表
                </p>
                <Table
                    columns={columns}
                ...
                />
                <p>
                    <Icon type="info" style={{color: '#1890ff'}}/>器材列表
                </p>
                <Table
