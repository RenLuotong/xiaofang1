...
class App extends Component {
  ...
  fetch = (
    ...
    $.ajax({
      ...
      url:
        SERVER +
        "huoquZhinengMensuoFangjian?page=" +
        page +
        "&size=" +
        size +
        "&fangjianmingcheng=" +
      ...
      success: function (data) {
        ...
        for (let i = 0; i < data.data.content.length; i++) {
          ...
          if (data.data.content[i]["fangjianleixing"] == "ZHINENGSUOFANGJIAN") {
            data.data.content[i]["fangjianleixing"] = "智能门锁房间";
          } else if (
            data.data.content[i]["fangjianleixing"] == "KAOQINJIFANGJIAN"
          ) {
            data.data.content[i]["fangjianleixing"] = "考勤机房间";
          } else if (
            data.data.content[i]["fangjianleixing"] == "JIASHUJIEDAISHI"
          ) {
            data.data.content[i]["fangjianleixing"] = "家属接待室房间";
  ...
  render() {
    let fangjianleixingListOptions = this.state.fangjianleixingList.map(
      (item) => (
