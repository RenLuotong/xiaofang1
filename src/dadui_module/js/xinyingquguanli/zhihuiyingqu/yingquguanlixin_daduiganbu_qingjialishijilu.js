...
import yingquguanlixin_daduiganbu_qingjialishijiluxiangqing from "./yingquguanlixin_daduiganbu_qingjialishijiluxiangqing";
const View = [];
        ...
        <Switch>
          ...
          <Route
            path={
              ...
              "/yingquguanlixin_daduiganbu_qingjialishijiluxiangqing/:shenqingdanbianhao"
            }
            component={yingquguanlixin_daduiganbu_qingjialishijiluxiangqing}
          />
        </Switch>
      </div>
    );
...
class App extends Component {
  ...
  getshenpiList(value) {
    ...
    $.ajax({
      type: "GET",
      url: SERVER + "huoqudaduizhangshenpizhunagtaiMeiju",
  ...
  tongguo(shenqingdanbianhao, processBaseId) {
    ...
    $.ajax({
      ...
      url:
        SERVER +
        "chaxunzhidingjueserenyuanBybanzhang?processBaseId=" +
        processBaseId,
      success: function (data) {
        let list = [];
        if (data.status != 0) {
  ...
  jujue(shenqingdanbianhao, processBaseId) {
    ...
    $.ajax({
      ...
      url:
        SERVER +
        "linshiqingjiashenpi?shenqingdanbianhao=" +
        shenqingdanbianhao +
        "&instanceStateType=" +
        instanceStateType +
        "&processBaseId=" +
  ...
  tijiao() {
    ...
    if (
      typeof shenpirenbianhao == "undefined" ||
      shenpirenbianhao == "" ||
      shenpirenbianhao == null
    ) {
  ...
  getJigouList(value) {
    ...
    $.ajax({
      type: "GET",
      url: SERVER + "daduiAllzhongduiListApp",
      success: function (data) {
        console.log(data);
        let list = [];
        if (data.status != 0) {
  ...
  render() {
    ...
    let shenpirenyuanOptions = this.state.shenpirenyuanList.map((item) => (
      <Select.Option key={item["key"]} value={item["renyuanbianhao"]}>
        {item["xingming"]}
      </Select.Option>
    ));

    const jizhouOptions = this.state.jigouList.map((item) => (
      <Select.Option key={item["id"]} value={item["jigoudaima"]}>
        {item["jigoumingcheng"]}
      </Select.Option>
    ));
    ...
    const columns = [
      ...
      },
      {
        ...
        render: (text, record, index) => {
          if (
            record["shenqingzhuangtai"] == "中队领导已批准" &&
            record["qingjialeixing"] == "年休"
          ) {
            return (
              <span>
                <Popconfirm
                  placement="topLeft"
                  title="确认要通过这条请假申请?"
                  onConfirm={this.tongguo.bind(
                    this,
                    record["shenqingdanbianhao"],
                    record["processBaseId"]
                  )}
                  okText="确认"
                  cancelText="取消"
                >
                ...
                <Link
                  to={
                    ...
                    "/yingquguanlixin_daduiganbu_qingjialishijiluxiangqing/" +
                    record["shenqingdanbianhao"]
                  }
                >
                  详情
            ...
            record["shenqingzhuangtai"] == "已同意" ||
            record["shenqingzhuangtai"] == "假期中" ||
            record["shenqingzhuangtai"] == "已销假" ||
            record["shenqingzhuangtai"] == "已超假"
          ) {
            return (
              <span>
                <a
                  href={
                    SERVER +
                    "generateReport?shenqingdanbianhao=" +
                    record.shenqingdanbianhao
                  }
                  title="打印"
                  target="_blank"
      ...
      <div>
        ...
        >
          <FormItem label="组织机构">
            <Select
              style={{ width: 200 }}
              value={jigoumingcheng}
              onChange={this.getJigouList.bind(this)}
            >
